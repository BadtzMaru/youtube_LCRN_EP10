...
import { StyleSheet, View, Text, TouchableOpacity, ScrollView, FlatList, Image, ImageBackground, LogBox } from 'react-native';
...
const Home = ({ navigation }) => {
			...
			<View style={{ width: '100%', height: 290, ...styles.shadow }}>
				<ImageBackground source={images.banner} resizeMode='cover' style={{ flex: 1, alignItems: 'center' }}>
					...
					<View style={{ alignItems: 'center', justifyContent: 'center' }}>
						...
						<Text style={{ margin: SIZES.base, color: COLORS.white, ...FONTS.h1 }}>${dummyData.portfolio.balance}</Text>
						<Text style={{ color: COLORS.white, ...FONTS.body5 }}>{dummyData.portfolio.changes} Last 24</Text>
					</View>
					{/* Trending */}
	...
	function renderNotice() {
		return (
			<View
				...
				<Text style={{ color: COLORS.white, ...FONTS.h3 }}>Inversting Safety</Text>
				<Text style={{ marginTop: SIZES.base, color: COLORS.white, ...FONTS.h4, lineHeight: 18 }}>
					It's very difficult to time an investment.It's very difficult to time an investment.It's very difficult to time an investment
				</Text>
				<TouchableOpacity style={{ marginTop: SIZES.base }} onPress={() => console.log('更多')}>
	...
	function renderTransactionHistory() {
		return <TransactionHistory customContainerStyel={{ ...styles.shadow }} history={transactionHistory} />;
	}
	return (
		<ScrollView>
